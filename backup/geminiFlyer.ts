...
export async function generateFlyerContent(options: GenerationOptions): Promise<GeminiResponse> {
  ...
  try {
    ...
    ULTRA IMPORTANT TECHNICAL REQUIREMENTS:
    1. KEEP YOUR HTML EXTREMELY SIMPLE - Use ONE MAIN DIV with absolute positioning for the text
    2. CRITICAL: For the main text (headlines), use ONLY: position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
    3. DO NOT nest elements unnecessarily - keep structure flat and simple
    4. AVOID creating containers that might interfere with text positioning
    5. Text should be directly on the body or in a single container div
    ...
    HTML STRUCTURE: 
    Make your HTML as simple as possible, for example with a single div:
    "<div class=\\"main-text\\">THE LAST BIRTHDAY</div>"
    
    TEXT STYLING REQUIREMENTS:
    - Use absolute positioning for text elements
    - Center headline with position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
    - Use viewport units for fonts (vw, vh) appropriately for the aspect ratio
    - Avoid nested containers or divs that would affect centering
    - Apply color analysis from background image when available
    
    EXAMPLE IDEAL CSS:
    Use styling like:
    ".main-text { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; }"
    
    Create a design that appears professional but with minimal HTML structure.`;
  ...
  try {
    ...
    const backgroundStyle = options.backgroundImageBase64 
      ? `
          ...
          /* Simplified structure - remove nested containers */
          .flyer-container {
            position: absolute;
            top: 0;
            left: 0;
    ...
    const fullHtml = `
      ...
      <head>
        ...
        <style>
          ...
          * {
          ...
          /* Special override to ensure text centering works */
          .main-text, .headline, h1, h2, h3 {
            position: absolute !important;
            top: 50% !important;
            left: 50% !important;
      ...
      <body>
        <!-- Simplified container structure to prevent div nesting problems -->
        ${htmlContent}
      </body>
      </html>
    `;
