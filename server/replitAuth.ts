...
export function getSession() {
  ...
  return session({
    secret: process.env.SESSION_SECRET || "flyer-generator-secret",
    store: sessionStore,
    resave: false,
...
async function upsertUser(
  ...
  return await storage.upsertUser({
    ...
    password: "", // Replit auth doesn't need a local password
    replit_id: claims["sub"],
    email: claims["email"],
    first_name: claims["first_name"],
    last_name: claims["last_name"],
    bio: claims["bio"],
    display_name: claims["username"], // Use the username as display name
    photo_url: claims["profile_image_url"],
  });
}
