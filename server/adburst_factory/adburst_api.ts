...
- Designed for maximum conversion and audience engagement`;
    ...
    // Customize the user prompt based on script length and duration
    let wordCount, structureGuidance;
    
    if (duration <= 10) {
      wordCount = "30-40 words (for a short 5-10 second video)";
      structureGuidance = `- Keep the script concise and impactful
      - Focus on a single key benefit or feature
      - Create immediate emotional impact`;
    } else if (duration <= 30) {
      wordCount = "80-120 words (for a medium 15-30 second video)";
      structureGuidance = `- Structure with clear introduction (hook), middle section (key benefits), and conclusion (call-to-action)
      - Include 2-3 distinct points that can be highlighted in different segments
      - Create natural transition points between ideas`;
    } else {
      wordCount = "180-240 words (for a long 45-60 second video)";
      structureGuidance = `- Create a compelling story arc with introduction, development, climax, and resolution
      - Include 4-6 distinct segments covering different aspects of the product
      - Build emotional connection through storytelling
      - Include multiple benefit points and social proof elements
      - Create natural transition points every 8-10 seconds for visual changes`;
    }
...
export async function processAdBurstRequest(req: Request, res: Response) {
  try {
    ...
    // Validate duration (5 seconds to 60 seconds)
